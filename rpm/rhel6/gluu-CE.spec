...
%post
/usr/sbin/chroot /opt/gluu-server-3.1.5 /bin/su - root -c 'sh /tmp/system_user.list'  > /dev/null 2>&1
/usr/sbin/chroot /opt/gluu-server-3.1.5 /bin/su - root -c 'sh /tmp/system_group.list'  > /dev/null 2>&1
/usr/sbin/chroot /opt/gluu-server-3.1.5 /bin/su - root -c 'rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY-GLUU' > /dev/null 2>&1
/usr/sbin/chroot /opt/gluu-server-3.1.5 /bin/su - root -c 'service iptables stop' > /dev/null 2>&1
...
/bin/cp -f /opt/gluu-server-3.1.5/tmp/profile /opt/gluu-server-3.1.5/etc/profile > /dev/null 2>&1
