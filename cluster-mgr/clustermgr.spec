...
cp -a %{SOURCE1} %{buildroot}/etc/systemd/system/clustermgr.service
...
rm -rf /etc/systemd/system/clustermgr.service > /dev/null 2>&1
systemctl daemon-reload > /dev/null 2>&1
rm -rf /usr/local/bin/clustermgr4-cli > /dev/null 2>&1
fi
...
%files
/opt/clustermgr/*
/etc/systemd/system/clustermgr.service
